...
function collapseContiguousByteRanges(chunks) {
  ...
  for (let x2, y2, i = 0, j2 = 0, n2 = chunks.length; ++i < n2; ) {
    x2 = result[j2];
    y2 = chunks[i];
...
var StructRow = class {
  ...
  toJSON() {
    ...
    const json = {};
    for (let j2 = -1, n2 = keys.length; ++j2 < n2; ) {
...
var MapRow = class {
  ...
  toJSON() {
    ...
    const json = {};
    for (let i = -1, n2 = keys.length; ++i < n2; ) {
...
function createArrayLikeComparator(lhs) {
  const comparators = [];
  for (let i = -1, n2 = lhs.length; ++i < n2; ) {
...
function compareObject(comparators, obj, keys) {
  ...
  for (; i < n2 && !lKey.done && !rKey.done && !rVal.done; ++i, lKey = lKeyItr.next(), rKey = rKeyItr.next(), rVal = rValItr.next()) {
    if (lKey.value !== rKey.value || !comparators[i](rVal.value)) {
      break;
    }
  }
  if (i === n2 && lKey.done && rKey.done && rVal.done) {
    return true;
  }
...
function sliceChunks(chunks, offsets, begin, end) {
  const slices = [];
  for (let i = -1, n2 = chunks.length; ++i < n2; ) {
...
function wrapChunkedIndexOf(indexOf) {
  ...
  function chunkedIndexOf(data, chunkIndex, fromIndex) {
    ...
    for (let i = chunkIndex - 1, n2 = data.length; ++i < n2; ) {
      const chunk = data[i];
...
function indexOfValue(data, searchElement, fromIndex) {
  ...
  for (let i = (fromIndex || 0) - 1, n2 = data.length; ++i < n2; ) {
  ...
  __string(offset, opt_encoding) {
    ...
    while (i < length) {
      ...
      if (a < 192) {
        ...
        const b2 = this.readUint8(offset + i++);
        if (a < 224) {
          codePoint = (a & 31) << 6 | b2 & 63;
        } else {
          const c = this.readUint8(offset + i++);
          if (a < 240) {
            codePoint = (a & 15) << 12 | (b2 & 63) << 6 | c & 63;
          } else {
            const d = this.readUint8(offset + i++);
            codePoint = (a & 7) << 18 | (b2 & 63) << 12 | (c & 63) << 6 | d & 63;
          }
        }
      }
      if (codePoint < 65536) {
    ...
    if (s instanceof Uint8Array) {
      ...
      while (i < s.length) {
        ...
        if (a < 55296 || a >= 56320) {
          ...
          const b2 = s.charCodeAt(i++);
          codePoint = (a << 10) + b2 + (65536 - (55296 << 10) - 56320);
        }
        if (codePoint < 128) {
          utf8.push(codePoint);
        } else {
...
function generateDictionaryMap(fields, dictionaries = /* @__PURE__ */ new Map()) {
  for (let i = -1, n2 = fields.length; ++i < n2; ) {
...
var Footer_ = class {
  ...
  *recordBatches() {
    for (let block, i = -1, n2 = this.numRecordBatches; ++i < n2; ) {
  ...
  *dictionaryBatches() {
    for (let block, i = -1, n2 = this.numDictionaries; ++i < n2; ) {
...
var Uint642 = class extends BaseInt64 {
  ...
  static convertArray(values) {
    ...
    for (let i = -1, n2 = values.length; ++i < n2; ) {
...
var FixedSizeListBuilder = class extends Builder {
  setValue(index, value) {
    ...
    for (let i = -1, n2 = value.length; ++i < n2; ) {
...
function distributeChildren(fields, batchLength, children, columns, memo) {
  ...
  for (let i = -1, n2 = columns.length; ++i < n2; ) {
...
var Table = class {
  ...
  select(columnNames) {
    const nameToIndex = this.schema.fields.reduce((m, f2, i) => m.set(f2.name, i), /* @__PURE__ */ new Map());
...
var TypeAssembler = class extends Visitor {
  ...
  visitDictionary(node, b2) {
    ...
    DictionaryEncoding.addId(b2, new Long3(node.id, 0));
...
var Message2 = class {
  ...
  static encode(message) {
    ...
    Message.addBodyLength(b2, new Long4(message.bodyLength, 0));
...
function decodeFieldNodes(batch) {
  const nodes = [];
  for (let f2, i = -1, j2 = -1, n2 = batch.nodesLength(); ++i < n2; ) {
    if (f2 = batch.nodes(i)) {
...
function decodeBuffers(batch, version) {
  const bufferRegions = [];
  for (let b2, i = -1, j2 = -1, n2 = batch.buffersLength(); ++i < n2; ) {
...
function decodeSchemaFields(schema, dictionaries) {
  const fields = [];
  for (let f2, i = -1, j2 = -1, n2 = schema.fieldsLength(); ++i < n2; ) {
...
function decodeFieldChildren(field, dictionaries) {
  const children = [];
  for (let f2, i = -1, j2 = -1, n2 = field.childrenLength(); ++i < n2; ) {
...
function decodeCustomMetadata(parent) {
  ...
  if (parent) {
    for (let entry, key, i = -1, n2 = Math.trunc(parent.customMetadataLength()); ++i < n2; ) {
...
function encodeField(b2, field) {
  ...
  if (!DataType.isDictionary(type)) {
    typeOffset = instance8.visit(type, b2);
  } else {
    typeId = type.dictionary.typeId;
    dictionaryOffset = instance8.visit(type, b2);
    typeOffset = instance8.visit(type.dictionary, b2);
  }
...
function encodeRecordBatch(b2, recordBatch) {
  ...
  RecordBatch2.addLength(b2, new Long4(recordBatch.length, 0));
...
function encodeDictionaryBatch(b2, dictionaryBatch) {
  ...
  DictionaryBatch.addId(b2, new Long4(dictionaryBatch.id, 0));
...
function encodeFieldNode(b2, node) {
  return FieldNode.createFieldNode(b2, new Long4(node.length, 0), new Long4(node.nullCount, 0));
}
function encodeBufferRegion(b2, node) {
  return Buffer.createBuffer(b2, new Long4(node.offset, 0), new Long4(node.length, 0));
}
...
function checkForMagicArrowString(buffer, index = 0) {
  for (let i = -1, n2 = MAGIC.length; ++i < n2; ) {
...
var RecordBatchWriter = class extends ReadableInterop {
  ...
  _writeBodyBuffers(buffers) {
    let buffer;
    let size, padding;
    for (let i = -1, n2 = buffers.length; ++i < n2; ) {
...
var J = (t, e, r, s) => {
  if (e && typeof e == "object" || typeof e == "function")
    for (let o of j(e))
      !K.call(t, o) && o !== r && h(t, o, { get: () => e[o], enumerable: !(s = Y(e, o)) || s.enumerable });
  return t;
};
var z = (t, e, r) => (r = t != null ? Q(H(t)) : {}, J(e || !t || !t.__esModule ? h(r, "default", { value: t, enumerable: true }) : r, t));
var x = V((ve, B) => {
  B.exports = Worker;
});
var $ = ((c) => (c[c.NONE = 0] = "NONE", c[c.DEBUG = 1] = "DEBUG", c[c.INFO = 2] = "INFO", c[c.WARNING = 3] = "WARNING", c[c.ERROR = 4] = "ERROR", c))($ || {});
var Z = ((d) => (d[d.NONE = 0] = "NONE", d[d.CONNECT = 1] = "CONNECT", d[d.DISCONNECT = 2] = "DISCONNECT", d[d.OPEN = 3] = "OPEN", d[d.QUERY = 4] = "QUERY", d[d.INSTANTIATE = 5] = "INSTANTIATE", d))(Z || {});
var X = ((d) => (d[d.NONE = 0] = "NONE", d[d.OK = 1] = "OK", d[d.ERROR = 2] = "ERROR", d[d.START = 3] = "START", d[d.RUN = 4] = "RUN", d[d.CAPTURE = 5] = "CAPTURE", d))(X || {});
var ee = ((c) => (c[c.NONE = 0] = "NONE", c[c.WEB_WORKER = 1] = "WEB_WORKER", c[c.NODE_WORKER = 2] = "NODE_WORKER", c[c.BINDINGS = 3] = "BINDINGS", c[c.ASYNC_DUCKDB = 4] = "ASYNC_DUCKDB", c))(ee || {});
var O = class {
  log(e) {
  }
};
...
var re = ((e) => (e[e.SUCCESS = 0] = "SUCCESS", e))(re || {});
var u = class {
  constructor(e, r) {
  ...
  async query(e) {
    ...
    let r = await this._bindings.runQuery(this._conn, e), s = RecordBatchReader.from(r);
    return console.assert(s.isSync()), console.assert(s.isFile()), new Table(s);
  }
  async send(e) {
    this._bindings.logger.log({ timestamp: new Date(), level: 2, origin: 4, topic: 4, event: 4, value: e });
    ...
    let s = new T(this._bindings, this._conn, r), o = await RecordBatchReader.from(s);
  ...
  async insertArrowTable(e, r) {
    let s = tableToIPC(e, "stream");
    await this.insertArrowFromIPCStream(s, r);
  }
...
var n = class {
  constructor(e, r) {
    ...
    this.type = e, this.data = r, this.promise = new Promise((s, o) => {
      this.promiseResolver = s, this.promiseRejecter = o;
    });
  }
};
function R(t) {
  switch (t.typeId) {
    ...
    case Type.List:
      return { sqlType: "list", valueType: R(t.valueType) };
    case Type.FixedSizeBinary:
    ...
    case Type.Utf8:
      return { sqlType: "utf8" };
    case Type.Struct:
      return { sqlType: "struct", fields: t.children.map((r) => p(r.name, r.type)) };
    case Type.Map: {
      let e = t;
    ...
    case Type.TimeSecond:
      return { sqlType: "time[s]" };
    case Type.Timestamp:
      return { sqlType: "timestamp", timezone: t.timezone || void 0 };
    case Type.TimestampSecond:
      return { sqlType: "timestamp[s]", timezone: t.timezone || void 0 };
    case Type.TimestampMicrosecond:
      return { sqlType: "timestamp[us]", timezone: t.timezone || void 0 };
    case Type.TimestampNanosecond:
      return { sqlType: "timestamp[ns]", timezone: t.timezone || void 0 };
    case Type.TimestampMillisecond:
      return { sqlType: "timestamp[ms]", timezone: t.timezone || void 0 };
  }
  throw new Error(`unsupported arrow type: ${t.toString()}`);
}
function p(t, e) {
  let r = R(e);
  return r.name = t, r;
...
var L = class {
  ...
  attach(e) {
    this._worker = e, this._worker.addEventListener("message", this._onMessageHandler), this._worker.addEventListener("error", this._onErrorHandler), this._worker.addEventListener("close", this._onCloseHandler), this._workerShutdownPromise = new Promise((r, s) => {
  ...
  async postTask(e, r = []) {
    ...
    let s = this._nextMessageId++;
    return this._pendingRequests.set(s, e), this._worker.postMessage({ messageId: s, type: e.type, data: e.data }, r), await e.promise;
  }
  onMessage(e) {
    let r = e.data;
    switch (r.type) {
    ...
    if (this._pendingRequests.delete(r.requestId), r.type == "ERROR") {
      ...
      o.name = r.data.name, o.stack = r.data.stack, s.promiseRejecter(o);
      return;
    }
    switch (s.type) {
      case "CLOSE_PREPARED":
    ...
    s.promiseRejecter(new Error(`unexpected response type: ${r.type.toString()}`));
  }
  onError(e) {
  ...
  async reset() {
    let e = new n("RESET", null);
    return await this.postTask(e);
  }
  async ping() {
    let e = new n("PING", null);
    await this.postTask(e);
  }
  async dropFile(e) {
    let r = new n("DROP_FILE", e);
    return await this.postTask(r);
  }
  async dropFiles() {
    let e = new n("DROP_FILES", null);
    return await this.postTask(e);
  }
  async flushFiles() {
    let e = new n("FLUSH_FILES", null);
    return await this.postTask(e);
  }
  async instantiate(e, r = null, s = (o) => {
  }) {
    this._onInstantiationProgress.push(s);
    let o = new n("INSTANTIATE", [e, r]);
    return await this.postTask(o);
  }
  async getVersion() {
    let e = new n("GET_VERSION", null);
    return await this.postTask(e);
  }
  async getFeatureFlags() {
    let e = new n("GET_FEATURE_FLAGS", null);
    return await this.postTask(e);
  }
  async open(e) {
    let r = new n("OPEN", e);
  ...
  async tokenize(e) {
    let r = new n("TOKENIZE", e);
    return await this.postTask(r);
  }
  async connectInternal() {
    let e = new n("CONNECT", null);
    return await this.postTask(e);
  }
  async connect() {
  ...
  async disconnect(e) {
    let r = new n("DISCONNECT", e);
    await this.postTask(r);
  }
  async runQuery(e, r) {
    let s = new n("RUN_QUERY", [e, r]);
    return await this.postTask(s);
  }
  async startPendingQuery(e, r) {
    let s = new n("START_PENDING_QUERY", [e, r]);
    return await this.postTask(s);
  }
  async pollPendingQuery(e) {
    let r = new n("POLL_PENDING_QUERY", e);
    return await this.postTask(r);
  }
  async cancelPendingQuery(e) {
    let r = new n("CANCEL_PENDING_QUERY", e);
    return await this.postTask(r);
  }
  async fetchQueryResults(e) {
    let r = new n("FETCH_QUERY_RESULTS", e);
    return await this.postTask(r);
  }
  async getTableNames(e, r) {
    let s = new n("GET_TABLE_NAMES", [e, r]);
    return await this.postTask(s);
  }
  async createPrepared(e, r) {
    let s = new n("CREATE_PREPARED", [e, r]);
    return await this.postTask(s);
  }
  async closePrepared(e, r) {
    let s = new n("CLOSE_PREPARED", [e, r]);
    await this.postTask(s);
  }
  async runPrepared(e, r, s) {
    let o = new n("RUN_PREPARED", [e, r, s]);
    return await this.postTask(o);
  }
  async sendPrepared(e, r, s) {
    let o = new n("SEND_PREPARED", [e, r, s]);
    return await this.postTask(o);
  }
  async globFiles(e) {
    let r = new n("GLOB_FILE_INFOS", e);
    return await this.postTask(r);
  }
  async registerFileText(e, r) {
    let s = se.encode(r);
    await this.registerFileBuffer(e, s);
  }
  async registerFileURL(e, r) {
    r === void 0 && (r = e);
    let s = new n("REGISTER_FILE_URL", [e, r]);
    await this.postTask(s);
  }
  async registerEmptyFileBuffer(e) {
    let r = new n("REGISTER_FILE_BUFFER", [e, new Uint8Array()]);
    await this.postTask(r);
  }
  async registerFileBuffer(e, r) {
    let s = new n("REGISTER_FILE_BUFFER", [e, r]);
    await this.postTask(s, [r.buffer]);
  }
  async registerFileHandle(e, r) {
    let s = new n("REGISTER_FILE_HANDLE", [e, r]);
    await this.postTask(s, []);
  }
  async collectFileStatistics(e, r) {
    let s = new n("COLLECT_FILE_STATISTICS", [e, r]);
    await this.postTask(s, []);
  }
  async exportFileStatistics(e) {
    let r = new n("EXPORT_FILE_STATISTICS", e);
    return await this.postTask(r, []);
  }
  async copyFileToBuffer(e) {
    let r = new n("COPY_FILE_TO_BUFFER", e);
    return await this.postTask(r);
  }
  async copyFileToPath(e, r) {
    let s = new n("COPY_FILE_TO_PATH", [e, r]);
    await this.postTask(s);
  }
  async insertArrowFromIPCStream(e, r, s) {
    if (r.length == 0)
      return;
    let o = new n("INSERT_ARROW_FROM_IPC_STREAM", [e, r, s]);
    await this.postTask(o, [r.buffer]);
  }
  async insertCSVFromPath(e, r, s) {
    if (s.columns !== void 0) {
      let c = [];
      for (let d in s.columns) {
        let m = s.columns[d];
        c.push(p(d, m));
      }
      s.columnsFlat = c, delete s.columns;
    }
    let o = new n("IMPORT_CSV_FROM_PATH", [e, r, s]);
    await this.postTask(o);
  }
  async insertJSONFromPath(e, r, s) {
    if (s.columns !== void 0) {
      let c = [];
      for (let d in s.columns) {
        let m = s.columns[d];
    ...
    let o = new n("IMPORT_JSON_FROM_PATH", [e, r, s]);
    await this.postTask(o);
  }
};
var F = class {
  ...
  failWith(e, r) {
    let s = { name: r.name, message: r.message, stack: r.stack || void 0 };
    this.postMessage({ messageId: this._nextMessageId++, requestId: e.messageId, type: "ERROR", data: s }, []);
  }
  async onMessage(e) {
    switch (e.type) {
      case "PING":
      ...
      case "INSTANTIATE":
        ...
        try {
          ...
          this._bindings = null, this.failWith(e, r);
        }
        return;
      default:
        break;
    ...
    try {
      switch (e.type) {
        ...
        case "CONNECT": {
          ...
          this.postMessage({ messageId: this._nextMessageId++, requestId: e.messageId, type: "CONNECTION_INFO", data: r.useUnsafe((s, o) => o) }, []);
          break;
        }
        case "DISCONNECT":
        ...
        case "START_PENDING_QUERY": {
          let r = this._bindings.startPendingQuery(e.data[0], e.data[1]), s = [];
          r && s.push(r.buffer), this.postMessage({ messageId: this._nextMessageId++, requestId: e.messageId, type: "QUERY_RESULT_HEADER_OR_NULL", data: r }, s);
          break;
        }
        case "POLL_PENDING_QUERY": {
          let r = this._bindings.pollPendingQuery(e.data), s = [];
          r && s.push(r.buffer), this.postMessage({ messageId: this._nextMessageId++, requestId: e.messageId, type: "QUERY_RESULT_HEADER_OR_NULL", data: r }, s);
          break;
        }
        case "CANCEL_PENDING_QUERY": {
      ...
      return this.failWith(e, r);
    }
  }
};
var C = async () => WebAssembly.validate(new Uint8Array([0, 97, 115, 109, 1, 0, 0, 0, 1, 4, 1, 96, 0, 0, 3, 2, 1, 0, 5, 3, 1, 0, 1, 10, 14, 1, 12, 0, 65, 0, 65, 0, 65, 0, 252, 10, 0, 0, 11]));
var U = async () => WebAssembly.validate(new Uint8Array([0, 97, 115, 109, 1, 0, 0, 0, 1, 4, 1, 96, 0, 0, 3, 2, 1, 0, 10, 8, 1, 6, 0, 6, 64, 25, 11, 11]));
var D = async () => WebAssembly.validate(new Uint8Array([0, 97, 115, 109, 1, 0, 0, 0, 1, 5, 1, 96, 0, 1, 123, 3, 2, 1, 0, 10, 10, 1, 8, 0, 65, 0, 253, 15, 253, 98, 11]));
var W = () => (async (t) => {
  try {
...
var M = () => G() ? "node" : navigator.userAgent;
var Ce = () => M().includes("Firefox");
var Ue = () => /^((?!chrome|android).)*safari/i.test(M());
function De() {
  let t = `https://cdn.jsdelivr.net/npm/${f}@${v}/dist/`;
  return { mvp: { mainModule: `${t}duckdb-mvp.wasm`, mainWorker: `${t}duckdb-browser-mvp.worker.js` }, eh: { mainModule: `${t}duckdb-eh.wasm`, mainWorker: `${t}duckdb-browser-eh.worker.js` } };
}
var g = null;
var y = null;
var b = null;
...
var N = null;
async function ie() {
  return g == null && (g = typeof BigInt64Array < "u"), y == null && (y = await U()), b == null && (b = await W()), S == null && (S = await D()), N == null && (N = await C()), { bigInt64Array: g, crossOriginIsolated: G() || globalThis.crossOriginIsolated || false, wasmExceptions: y, wasmSIMD: S, wasmThreads: b, wasmBulkMemory: N };
}
async function We(t) {
  let e = await ie();
  if (e.wasmExceptions) {
    if (e.wasmSIMD && e.wasmThreads && e.crossOriginIsolated && t.coi)
      return { mainModule: t.coi.mainModule, mainWorker: t.coi.mainWorker, pthreadWorker: t.coi.pthreadWorker };
    if (t.eh)
      return { mainModule: t.eh.mainModule, mainWorker: t.eh.mainWorker, pthreadWorker: null };
  }
  return { mainModule: t.mvp.mainModule, mainWorker: t.mvp.mainWorker, pthreadWorker: null };
}
var q = z(x());
async function Ge(t) {
  let e = new Request(t), r = await fetch(e), s = URL.createObjectURL(await r.blob());
  return new q.default(s);
}
export {
  L as AsyncDuckDB,
  u as AsyncDuckDBConnection,
  F as AsyncDuckDBDispatcher,
  ...
  Ae as PACKAGE_VERSION_MAJOR,
  we as PACKAGE_VERSION_MINOR,
  Pe as PACKAGE_VERSION_PATCH,
  re as StatusCode,
  O as VoidLogger,
  w as WorkerRequestType,
  P as WorkerResponseType,
  n as WorkerTask,
  Ge as createWorker,
  De as getJsDelivrBundles,
  de as getLogEventLabel,
  ae as getLogLevelLabel,
  le as getLogOriginLabel,
  ce as getLogTopicLabel,
  ie as getPlatformFeatures,
  Ce as isFirefox,
  G as isNode,
  Ue as isSafari,
  We as selectBundle
